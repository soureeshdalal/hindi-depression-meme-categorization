"""
phase2_v2.1.py  —  Label prediction with rebalanced calibration (Run V2.1)
---------------------------------------------------------------------------
Change from V2:
  - Calibration reminder "lean toward NO" replaced with balanced threshold:
    mark YES when symptom is a meaningful theme, NO when absent/incidental.
  - Fixes V2's under-prediction (low recall) without sacrificing precision.

Input : gemini_runs/run_v2/phase1_explanations_v2.csv   (same Phase 1 data)
Output: gemini_runs/run_v2.1/phase2_predictions_v2.1.csv

Usage:
    GEMINI_API_KEY=... python gemini_runs/run_v2.1/phase2_v2.1.py [--rpm 10]
"""
...
OUT_DIR = Path("gemini_runs/run_v2.1")
OUT_CSV = OUT_DIR / "phase2_predictions_v2.1.csv"
RETRIES = 3
RPM     = 10
...
- Feeling Down: Must be a meaningful theme — persistent sadness, hopelessness,
  or emptiness. Pure humour/sarcasm = NO.
- Lack of Interest: Requires BEHAVIOURAL evidence — stopped doing things or
  finds nothing enjoyable/worthwhile.
- Low Self-Esteem: Explicit worthlessness/guilt only — not general negativity
  or embarrassment.
- Decision threshold: if the symptom is a clear, meaningful theme in the meme
  (even if not the only theme), mark YES. Mark NO only if the symptom is
  absent or merely incidental background noise.
...
def main():
    ...
    print(f"\n{'='*58}")
    print(f"  Phase 2 V2.1 — Rebalanced calibration")
    print(f"{'='*58}")
